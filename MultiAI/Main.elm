...
update msg model =
    case msg of
        ExpectMsg msg ->
            let
                ( aiModel, aiCmd ) =
                    AI.Core.update msg model.expect
            in
                ( { model | expect = aiModel }, aiCmd |> Cmd.map ExpectMsg )

        RandomMsg msg ->
            let
                ( aiModel, aiCmd ) =
                    AI.Core.update msg model.random
            in
                ( { model | random = aiModel }, aiCmd |> Cmd.map RandomMsg )

        LearnMsg msg ->
            let
                ( aiModel, aiCmd ) =
                    AI.Core.update msg model.learn
            in
                ( { model | learn = aiModel }, aiCmd |> Cmd.map LearnMsg )
