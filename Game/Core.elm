...
update msg model =
    case msg of
        ...
        Expose row column ->
            ( expose model row column, Cmd.none )
...
exposePlaying board row column =
    ...
    in
        if hasExposedMine board' then
            Lost board'
        else if allTargetsExposed board' then
            Won board'
        else
            Playing board'
    ...
    let
        targetPoss =
            board.targets
                ...
                |> Set.fromList
    in
        Set.diff targetPoss board.exposed
            |> Set.isEmpty
...
score : Board -> Int
score board =
    if Set.isEmpty board.exposed then
        0
    else
        board.exposed
            |> Set.toList
...
cellValue board row column =
    if Set.member ( row, column ) board.mines then
        0
    else
        case Dict.get ( row, column ) board.targets of
            Just v ->
                v
