...
action board =
    ...
    in
        case response of
            ...
            Nothing ->
                crash "AI didn't produce an answer"


expectation : Board -> ( Int, Int ) -> Float
expectation board ( row, col ) =
    if Set.member ( row, col ) board.exposed then
        cellValue board row col |> toFloat
    else if size - rowMines board row == 0 || size - colMines board col == 0 then
        0.0
    else
        min (toFloat (rowAvailablePoints board row) / toFloat (rowAvailableSpaces board row))
...
update msg model =
    case ( msg, model.gameModel ) of
        ...
        ( GameMsg gameMsg, _ ) ->
            let
                ( gameModel, gameCmd ) =
                    Game.Core.update gameMsg model.gameModel
            in
                ( { model | gameModel = gameModel }, gameCmd |> Cmd.map GameMsg )
...
subscriptions model =
    if model.playing then
        Sub.batch
            [ Game.Core.subscriptions model.gameModel
